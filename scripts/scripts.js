...
/**
 * Applies `section-metadata` blocks to their parent section: each row is a
 * key/value pair; `style` becomes space-separated classes on the section,
 * any other key becomes a `data-*` attribute. The metadata block is removed.
...
function decorateSectionMetadata(main) {
  main.querySelectorAll(':scope > .section > div > .section-metadata').forEach((meta) => {
    ...
    // Remove the metadata block and its now-empty default-content wrapper
